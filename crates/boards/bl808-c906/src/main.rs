...
pub extern "C" fn kernel_main() -> ! {
    // Initialize platform
    let mut platform = Bl808C906Platform::new();
    platform.init();

    // Get serial port
    let mut uart = platform.serial_port();
    uart.init();
    uart.write_str("BL808 C906 (riscv64imac) - High-Performance 64-bit Core\r\n");
    uart.write_str("Platform: PLACEHOLDER - Awaiting memory map and peripheral definitions\r\n");
    uart.write_str("Note: This core typically runs Linux, but can run bare-metal kernels\r\n");
